...
static bool CheckCacheDirectory(const char* dir)
{
    if (pb_access(dir, 0) != 0) {
        if (pb_mkdir(dir) == -1) return false;
    }
    return true;
}
...
bool GetCacheName(const char* fname, const char* ext_name, char* cname)
{
    ...
    // Controllo esistenza directory delle cache
    if (cname[0] != 0) strcat(cname, DIR_SEP);
...
static bool SaveBBoxCache(const char* fname, const Box3<ScalarType>& boxOut)
{
    ...
    Box3d box;
    box.Import(boxOut);
    if (!GetCacheName(fname, bboxcacheext, d)) return false;
...
bool ScanBBox(const char* fname, Box3<ScalarType>& box, bool use_cache = true)
{
    ...
    if (use_cache) {
        if (CheckBBoxCache(fname, box)) return true;
    }
    ...
    for (int i = 0; i < int(pf.elements.size()); ++i) {
        ...
        if (!strcmp(pf.ElemName(i), "vertex")) {
            ...
            for (int j = 0; j < n; ++j)
                // pf.Read( 0 ); // prima era cosi' e faceva un'assert e scrivema plausibilimente a
                // caso in mem
                pf.Read(dummyspace);
        }
    }
...
template<class ScalarType>
bool ScanBBox(const char* fname, Box3<ScalarType>& box, const Matrix44<ScalarType>& m,
              bool use_cache, const char* matrixfname)
{
