...
Improved the compatibility for ply format for faces having the list size (e.g. number of vertexes of
a face) as a char instead of a uchar. Added a couple of new face descriptors, corrected a bug in
error reporting function (and restructured) and translated a few comments. Thanks to Patrick Min for
the careful bug reporting
...
public:
    PropDescriptor() {}
    PropDescriptor(std::string elemname, std::string propname, int stotype1, int memtype1,
                   size_t offset1, bool islist, bool alloclist, int stotype2, int memtype2,
                   size_t offset2, int format)
        : elemname(elemname)
        , propname(propname)
        , stotype1(stotype1)
        , memtype1(memtype1)
...
public:
    ...
    int AddToRead(const char* propname, int stotype1, int memtype1, size_t offset1, int islist,
                  int alloclist, int stotype2, int memtype2,
...
public:
    ...
    int AddToRead(const char* elemname, const char* propname, int stotype1, int memtype1,
    ...
    {
        return AddToRead(p.elemname.c_str(),
                         p.propname.c_str(),
                         p.stotype1,
                         p.memtype1,
                         p.offset1,
