...
template<class TriMeshType> class MeshCache
{
    class Pair
    {
    public:
...
public:
    ...
    ~MeshCache()
    {
        ...
        for (mi = MV.begin(); mi != MV.end(); ++mi) delete (*mi).M;
    }
    ...
    bool Find(std::string& name, TriMeshType*& sm)
    {
        ...
        typename std::list<Pair>::iterator
            oldest;   // quello che e' piu' tempo che non viene acceduto.
        int last;
...
template<class TriMeshType> class SimpleMeshProvider
{
...
public:
    ...
    {
        if (newsize == MC.MaxSize) return MC.MaxSize;
    ...
    bool openALN(const char* alnName)
    {
        ...
        //          AddSingleMesh(rmaps[i].filename.c_str(), rmaps[i].transformation,
        //          rmaps[i].quality);
    ...
    bool Find(int i, TriMeshType*& sm) { return MC.Find(meshnames[i], sm); }
    ...
    bool InitBBox()
    {
        ...
        for (int i = 0; i < int(meshnames.size()); ++i) {
            ...
            Id.SetIdentity();
            mt.Import(TrV[i]);
            printf("bbox scanning %4i/%i [%16s]      \r",
                   i + 1,
                   (int)meshnames.size(),
                   meshnames[i].c_str());
            if (tri::io::Importer<TriMeshType>::FileExtension(meshnames[i], "PLY") ||
                tri::io::Importer<TriMeshType>::FileExtension(meshnames[i], "ply")) {
                if (!(TrV[i] == Id))
...
class SFace;
class SUsedTypes
    : public vcg::UsedTypes<vcg::Use<SVertex>::AsVertexType, vcg::Use<SFace>::AsFaceType>
{};

class SVertex : public Vertex<SUsedTypes, vertex::Coord3f, vertex::Normal3f, vertex::VFAdj,
                              vertex::BitFlags, vertex::Color4b, vertex::Qualityf>
{};
class SFace : public Face<SUsedTypes, face::VertexRef, face::Normal3f, face::Qualityf, face::VFAdj,
                          face::BitFlags>
{};
